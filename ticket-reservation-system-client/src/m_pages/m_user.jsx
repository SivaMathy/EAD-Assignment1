...
export default function Users(peoples) {
  useEffect(() => {
    axios
      .get("http://192.168.8.164:88/api/User")
      .then((response) => {
  ...
  const toggleStatus = (personId, person) => {
    ...
    axios
      .put(`http://192.168.8.164:88/api/User/${personId}?Id=${personId}&Name=${person.name}&Email=${person.email}&Phone=${person.phone}&Role=${person.role}&Status=${newStatus}&Password=${person.password}`, {
        status: newStatus,
        name: person.name,
        id: person.id,
        email: person.email,
